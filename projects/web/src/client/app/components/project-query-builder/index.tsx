...
export class ProjectQueryBuilder extends React.PureComponent<IProjectQueryBuilderProps, IProjectQueryBuilderState> {
  ...
  render(): JSX.Element {
    return (
      <ListQueryBuilder items={this.props.projects} keyForItem={(item: IProject) => item.id} titleForItem={(item: IProject) => item.name} query={this.props.query} queryItem={this.props.queryItem} queryItemType="Project" itemToQueryItem={asEntity} itemComparer={entityComparer} onChange={this.props.onChange} />
    );
  }
}
