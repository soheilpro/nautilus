...
export class ProjectSelect extends React.PureComponent<IProjectSelectProps, IProjectSelectState> {
  render(): JSX.Element {
    return (
      <Select className={classNames('project-select-component', this.props.className)} selectedItem={this.props.project} items={this.props.projects} keyForItem={(item: IProject) => item.id} titleForItem={(item: IProject) => item.name} onChange={this.props.onChange} />
    );
  }
}
