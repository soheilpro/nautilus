...
export class ItemTypeRouter extends RouterBase<IItemType, IItemTypeChange> {
  ...
  async entityFromParams(params: IParams, request: IRequest): Promise<IItemType> {
    return {
  ...
  async changeFromParams(params: IParams, request: IRequest): Promise<IItemTypeChange> {
    return {
  ...
  entityToModel(entity: IItemType): ItemTypeModel {
    if (!entity)
      return undefined;
